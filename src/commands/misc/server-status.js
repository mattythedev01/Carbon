...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Let's see what the status of the members are in this guild"
    )
  ...
  run: async (client, interaction) => {
    ...
    guild.members.fetch({ withPresences: true }).then((fetchedMembers) => {
      ...
      if (status === `Online`) {
        interaction.reply({
          content: `${totalOnline.size} are online in this guild!`,
          ephemeral: true,
        });
        console.log(
      ...
      if (status === `Offline`) {
        interaction.reply({
          content: `${totalOffline.size} are offline in this guild!`,
          ephemeral: true,
        });
        console.log(
